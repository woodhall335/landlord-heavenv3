...
type CannibalisationDecision = {
  group_key: string;
  primary_url: string;
  competing_urls: string[];
  action: 'canonicalize' | 'differentiate_intent' | 'merge_or_prune';
  rationale: string;
};
...
const OUT_DIR = auditOutDir('cannibalisation', { latest: true });

const REQUIRED_DECISIONS: Array<[string, string, string]> = [
  ...
  try {
    ...
    for (const [idx, line] of lines.entries()) {
      try {
        ...
        warn(`Skipping invalid JSONL line ${idx + 1} in ${filePath}`);
      }
    }
    return rows;
  } catch (error) {
...
function normalizeUrl(urlLike: string): string {
  if (!urlLike) return '';
  try {
...
function pageTypeFromPath(urlPath: string) {
...
function normalizeRow(raw: Record<string, unknown>): NormalizedPage | null {
  ...
  const h1_texts = asStringArray(raw.h1_texts).length
    ? asStringArray(raw.h1_texts)
    : asStringArray(raw.h1Text || raw.h1 || raw.h1s);
  ...
  const jsonld_types = asStringArray(raw.jsonld_types).length
    ? asStringArray(raw.jsonld_types)
  ...
  const ctas = asStringArray(raw.ctas).length
    ? asStringArray(raw.ctas)
    : asStringArray(raw.cta_texts || raw.cta || raw.calls_to_action);
...
function scoreRichness(rows: NormalizedPage[]) {
  let score = 0;
...
async function loadSource(baseDir: string, fileName: string): Promise<SourceResult | null> {
  ...
  if (fileName.endsWith('.jsonl')) {
    ...
    if (Array.isArray(payload)) rawRows = payload;
      ...
      if (Array.isArray(obj.pages)) rawRows = obj.pages;
...
function mergePagesByUrl(primary: NormalizedPage[], fallback: NormalizedPage[]) {
...
function keywordBucket(urlPath: string) {
  return urlPath
    .replace(/^\//, '')
    .replace(/^products\//, '')
    .replace(/-template$/g, '')
...
function buildDecisions(pages: NormalizedPage[]): CannibalisationDecision[] {
  ...
  const decisions: CannibalisationDecision[] = [];
  ...
  for (const [a, b, key] of REQUIRED_DECISIONS) {
    ...
    const fallbackDecision: CannibalisationDecision = {
      group_key: groupKey,
      primary_url: primary,
      competing_urls: [a],
...
function toCanonicalActionRows(decisions: CannibalisationDecision[]) {
  const rows = [
...
async function getInputDir() {
  ...
  const baseFiles = await fs.readdir(SEO_BASE_DIR).catch(() => [] as string[]);
  ...
  const dirs = await fs.readdir(SEO_BASE_DIR, { withFileTypes: true }).catch(() => [] as Awaited<ReturnType<typeof fs.readdir>>);
  const timestampDirs = dirs.filter((d: any) => d.isDirectory() && d.name !== 'latest').map((d: any) => d.name).sort();
...
async function main() {
  ...
  // Source precedence (richest first):
  // 1) onpage.jsonl (usually has content + structure + CTA-rich data)
  // 2) seo_audit_report.json (deep-seo-audit.ts standard output with indexability/canonical/H1/JSON-LD)
  // 3) crawl_table.jsonl
  // 4) pages.jsonl
  // 5) report.json
  const sourceOrder = ['onpage.jsonl', 'seo_audit_report.json', 'crawl_table.jsonl', 'pages.jsonl', 'report.json'];

  const loaded = (await Promise.all(sourceOrder.map((file) => loadSource(inputDir, file)))).filter(Boolean) as SourceResult[];
  if (loaded.length === 0) {
