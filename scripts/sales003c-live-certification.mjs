...
function classifyNetworkRow(row) {
  const url = new URL(row.url, baseUrl);
  ...
  if (url.pathname === '/checklists/ground-1a.pdf' && (row.status === 404 || row.status === 0)) {
    return { classification: 'genuine_linked_resource_404', result: 'FAIL' };
  }
  if (['image', 'font', 'stylesheet', 'script', 'document'].includes(row.resource_type)) {
    return { classification: 'genuine_page_asset_failure', result: 'FAIL' };
  }
  if (url.pathname.startsWith('/api/')) return { classification: 'application_api_failure', result: 'FAIL' };
  return { classification: 'other_request_failure', result: 'FAIL' };
}

async function installQaIdentity(page, suffix) {
  await page.evaluateOnNewDocument((identity) => {
    window.__sales003cQaAnalytics = [];
    const originalBeacon = navigator.sendBeacon?.bind(navigator);
    if (originalBeacon) {
      navigator.sendBeacon = (url, data) => {
        if (String(url).includes('/api/analytics/events') && data instanceof Blob) {
          void data.text().then((text) => {
            try { window.__sales003cQaAnalytics.push(JSON.parse(text)); } catch {}
          });
        }
    ...
    window.fetch = (input, init) => {
      if (String(input).includes('/api/analytics/events') && typeof init?.body === 'string') {
        try { window.__sales003cQaAnalytics.push(JSON.parse(init.body)); } catch {}
      }
    ...
    window.localStorage.setItem('lh_lead_email', 'sales003c-qa@example.invalid');
  }, `${qaMarker}-${suffix}`);
}
...
async function runCheckpoint(name, producer) {
  const file = path.join(root, 'tmp', 'sales003c', `${name}.json`);
...
async function resetToFirstViewport(page) {
  ...
  if (position.x !== 0 || position.y !== 0) {
    throw new Error(`First-viewport reset failed: scrollX=${position.x}, scrollY=${position.y}`);
  }
  return position;
}

async function prepareFirstViewport(page) {
  await page.waitForFunction(() => document.readyState === 'complete', { timeout: 60_000 });
  await page.evaluate(async () => {
    if (document.fonts?.ready) await document.fonts.ready;
  });
  await sleep(250);
  return resetToFirstViewport(page);
}

async function triggerLazyMediaAndStabilize(page) {
  await page.evaluate(() => {
  ...
  await resetToFirstViewport(page);
  await sleep(250);
  return resetToFirstViewport(page);
}
...
async function auditRouteViewport(browser, route, viewport) {
  ...
  try {
    ...
    const initialPosition = await prepareFirstViewport(page);
    const slug = safeSlug(route);
    const firstFile = `${slug}-${viewport.name}-first.png`;
    const firstBefore = await page.evaluate(() => window.scrollY);
    await page.screenshot({ path: path.join(screenshotDir, firstFile), fullPage: false });
    const firstAfter = await page.evaluate(() => window.scrollY);
    screenshots.push({
      route, viewport: viewport.name, width: viewport.width, capture: 'first-viewport',
      file: `screenshots/${firstFile}`, scroll_y_before: firstBefore, scroll_y_after: firstAfter,
      result: initialPosition.y === 0 && firstBefore === 0 && firstAfter === 0 ? 'PASS' : 'FAIL',
    });

    await triggerLazyMediaAndStabilize(page);
    const fullFile = `${slug}-${viewport.name}-full.png`;
    const fullBefore = await page.evaluate(() => window.scrollY);
    await page.screenshot({ path: path.join(screenshotDir, fullFile), fullPage: true });
    const fullAfter = await page.evaluate(() => window.scrollY);
    screenshots.push({
      route, viewport: viewport.name, width: viewport.width, capture: 'full-page',
      file: `screenshots/${fullFile}`, scroll_y_before: fullBefore, scroll_y_after: fullAfter,
      result: fullBefore === 0 && fullAfter === 0 ? 'PASS' : 'FAIL',
    });
  } catch (error) {
  ...
  const classifiedNetwork = capture.networkRows.map((row) => ({ ...row, ...classifyNetworkRow(row) }));
  const criticalNetwork = classifiedNetwork.filter((row) => row.result === 'FAIL');
  const fatalConsole = capture.consoleRows.filter((row) => row.type === 'error' || row.type === 'pageerror');
  const hydrationMessages = capture.consoleRows.filter((row) => /hydration|react.*418|server.*client/i.test(row.text));
  ...
  const screenshotPass = screenshots.length === 2 && screenshots.every((row) => row.result === 'PASS');
  const heroPass = !productRoutes.has(route) || viewport.name !== 'mobile' || Boolean(dom && dom.contentTopToPrimaryCta >= 0 && dom.contentTopToPrimaryCta <= 650 && dom.heroHeight <= 1150);
  const heroActions = dom?.actionGroups.filter((action) => action.top <= (dom?.heroHeight || 0) + 30) || [];
  const duplicateCtaPass = !productRoutes.has(route) || (
    heroActions.filter((action) => action.ancestry.includes('hero-btn-primary')).length === 1 &&
    heroActions.filter((action) => action.ancestry.includes('hero-btn-secondary')).length <= 1
  );
  const gapPass = Boolean(dom && dom.emptyLayoutContainers === 0 && dom.largestGap.size <= 260);
  const hydrationPass = hydrationMessages.length === 0 && !capture.consoleRows.some((row) => row.type === 'pageerror');
  const routePass = Boolean(
    ...
    screenshotPass && duplicateCtaPass && heroPass && gapPass && hydrationPass &&
    accessibilityPass &&
  ...
  const qaRow = {
    ...
    screenshot_result: screenshotPass ? 'PASS' : 'FAIL',
    duplicate_cta_result: duplicateCtaPass ? 'PASS' : 'FAIL',
    hero_height_px: productRoutes.has(route) ? (dom?.heroHeight ?? -1) : '',
    hero_result: heroPass ? 'PASS' : 'FAIL',
    largest_vertical_gap_px: dom?.largestGap.size ?? -1,
    gap_result: gapPass ? 'PASS' : 'FAIL',
    media_result: inspected.mediaRows.every((row) => row.result === 'PASS') ? 'PASS' : 'FAIL',
    hydration_result: hydrationPass ? 'PASS' : 'FAIL',
    console_result: fatalConsole.length === 0 ? 'PASS' : 'FAIL',
    network_result: criticalNetwork.length === 0 ? 'PASS' : 'FAIL',
    accessibility_result: accessibilityPass ? 'PASS' : 'FAIL',
    keyboard_result: keyboard.result,
    navigation_error: navigationError,
    full_screenshot: screenshots.find((row) => row.capture === 'full-page')?.file || '',
    first_viewport_screenshot: screenshots.find((row) => row.capture === 'first-viewport')?.file || '',
  ...
  await context.close();
  return { qaRow, accessibilityRow, screenshots, mediaRows: inspected.mediaRows, consoleRows: capture.consoleRows, networkRows: classifiedNetwork, dom };
}
...
async function auditProductCtas(browser, route) {
  ...
  await prepareFirstViewport(page);
...
async function auditHmo() {
  ...
  for (const config of scenarios) {
    ...
    await sleep(500);
    const recordedAnalytics = await page.evaluate(() => window.__sales003cQaAnalytics || []);
    ...
    const noPii = !payloadText.includes('SW1A') && !payloadText.includes('sales003c-qa@example.invalid');
...
async function auditArrears(browser) {
  ...
  rows.push({
    ...
    cumulative_result_event_count: await page.evaluate(() => (window.__sales003cQaAnalytics || []).filter((item) => item?.eventName === 'result_viewed').length),
...
async function auditExperiment(browser) {
  ...
  for (const identity of identities) {
    ...
    const analyticsExperiment = await page.evaluate(() =>
      (window.__sales003cQaAnalytics || []).map((item) => item?.payload?.experimentId).filter(Boolean)
    );
...
async function auditAnalyticsIngestion(browser) {
  ...
  const rows = expected.map(([label, eventName, toolName]) => {
    ...
    return {
      ...
      canonical_event_names: matches.map((item) => item.responseBody?.canonicalEventName || '').join('|'),
      persisted_event_names: matches.map((item) => item.responseBody?.persistedEventName || '').join('|'),
...
async function auditGroundResource() {
  const url = `${baseUrl}/checklists/ground-1a.pdf`;
  let status = 0;
  let mime = '';
  let bytes = 0;
  let signature = '';
  ...
  try {
    ...
    bytes = data.length;
    signature = data.subarray(0, 5).toString('ascii');
  } catch (caught) {
    error = caught.message;
  }
  const pageResponse = await fetch(`${baseUrl}/section-8-grounds/how-to-evict-a-tenant-using-ground-1a`);
  const pageHtml = await pageResponse.text();
  const linked = /href=["']\/checklists\/ground-1a\.pdf["']/i.test(pageHtml);
  const accurateLabel = /href=["']\/checklists\/ground-1a\.pdf["'][^>]*>[^<]*(?:Ground 1A|checklist)/i.test(pageHtml)
    || /(?:Ground 1A|checklist)[\s\S]{0,300}href=["']\/checklists\/ground-1a\.pdf["']/i.test(pageHtml);
  const pass = status === 200 && /application\/pdf/i.test(mime) && bytes > 100_000 && signature === '%PDF-' && linked && accurateLabel;
  return [{
    resource_url: url,
    http_status: status,
    mime_type: mime,
    ...
    meaningful_content_threshold: bytes > 100_000 ? 'PASS' : 'FAIL',
    linked_from_ground_1a_page: linked ? 'yes' : 'no',
    accessible_label_accurate: accurateLabel ? 'yes' : 'no',
    error,
    result: pass ? 'PASS' : 'FAIL',
  }];
}
...
async function main() {
  ...
  try {
    if (process.env.QA_ONLY_ANALYTICS === '1') {
      const analytics = await auditAnalyticsIngestion(browser);
      await writeCsv('analytics-persistence-final.csv', analytics.rows, Object.keys(analytics.rows[0]));
    ...
    const analytics = await runCheckpoint('analytics-audit', () => auditAnalyticsIngestion(browser));
    const groundRows = await runCheckpoint('ground-resource-audit', auditGroundResource);
    ...
    await writeCsv('media-load-final.csv', all.flatMap((item) => item.mediaRows), ['route','viewport','source','http_result','complete_state','natural_width','natural_height','lazy_load_state','rendered_width','rendered_height','result']);
    await writeCsv('browser-console-errors.csv', all.flatMap((item) => item.consoleRows), ['route','viewport','type','text','result']);
    await writeCsv('network-classification.csv', all.flatMap((item) => item.networkRows), ['route','viewport','url','method','resource_type','status','failure','classification','result']);
    await writeCsv('accessibility-final.csv', all.map((item) => item.accessibilityRow), Object.keys(all[0].accessibilityRow));
    const screenshotRows = all.flatMap((item) => item.screenshots);
    await writeCsv('screenshot-index.csv', screenshotRows, ['route','viewport','width','capture','file','scroll_y_before','scroll_y_after','result']);
    await writeCsv('screenshot-scroll-position.csv', screenshotRows, ['route','viewport','capture','file','scroll_y_before','scroll_y_after','result']);
    await writeCsv('duplicate-cta-final.csv', ctaRows, Object.keys(ctaRows[0]));
    const mobileProducts = all.filter((item) => item.qaRow.viewport === 'mobile' && productRoutes.has(item.qaRow.route));
    await writeCsv('product-hero-height-final.csv', mobileProducts.map((item) => ({
      ...
      primary_cta_at_or_above_650px: (item.dom?.contentTopToPrimaryCta ?? 99999) <= 650 ? 'yes' : 'no',
      decision_section_at_or_below_1150px: (item.dom?.heroHeight ?? 99999) <= 1150 ? 'yes' : 'no',
      exception: '',
      result: item.dom && item.dom.contentTopToPrimaryCta >= 0 && item.dom.contentTopToPrimaryCta <= 650 && item.dom.heroHeight <= 1150 ? 'PASS' : 'FAIL',
    })), ['route','viewport_width','content_top_to_primary_cta_px','hero_height_px','viewport_heights_before_preview','primary_cta_at_or_above_650px','decision_section_at_or_below_1150px','exception','result']);
    await writeCsv('excessive-gap-final.csv', all.filter((item) => item.qaRow.viewport === 'mobile').map((item) => ({
      route: item.qaRow.route,
      ...
      intentional_layout_reason: item.dom && item.dom.largestGap.size > 120 && item.dom.largestGap.size <= 260
        ? 'Intentional section spacing between populated blocks; no hidden or empty media-height container is present.'
        : '',
      result: item.dom && item.dom.largestGap.size <= 260 && item.dom.emptyLayoutContainers === 0 ? 'PASS' : 'FAIL',
    })), ['route','largest_vertical_gap_px','gap_location','gap_over_120px','hidden_or_empty_media_height_count','intentional_layout_reason','result']);
    await writeCsv('hmo-form-position-final.csv', hmoRows.filter((row) => row.scenario === 'initial_page').map((row) => ({
      route: '/tools/hmo-license-checker',
      viewport_width: 390,
      checker_top_mobile_px: row.checker_top_mobile_px,
      target_max_px: 700,
      result: row.checker_top_mobile_px <= 700 ? 'PASS' : 'FAIL',
    })), ['route','viewport_width','checker_top_mobile_px','target_max_px','result']);
    ...
    await writeCsv('hydration-final.csv', qaRows.map((row) => {
