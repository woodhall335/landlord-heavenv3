    ...
    if (isCourtReady === false) {
      return (
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
          ...
          {isMoneyClaimFlow ? 'Needs attention' : 'Needs attention before use'}
        </span>
      );
    }
    return null;
      ...
      {hasBlockingIssues && (
        <Card className="p-6 border-red-200 bg-red-50">
          ...
          <p className="text-sm text-red-800 mb-4">
            These issues will stop the document from being safe to rely on yet.
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
          ...
          <div>
            ...
            {(analysis.red_flags?.length > 0 || analysis.compliance_issues?.length > 0) && (
              <p className="text-sm text-amber-700 mt-2">
                ?? See &quot;Things to Fix or Improve&quot; below for compliance concerns.
              </p>
            )}
          </div>
        )}
      ...
      <p className="text-xs text-center text-gray-500 mt-2">
        {isPaid
          ...
          : 'We will still generate the full pack, but use the guidance first so you reach a safe position before serving or issuing anything.'}
      </p>
    </div>
  );
}
