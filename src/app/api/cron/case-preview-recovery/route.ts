/**
 * Preview-abandoned case recovery cron.
 *
 * Sends day-1 and day-7 restart emails for cases that reached preview/review
 * but have not created a paid order or final documents.
 */
...
const JOB_NAME = 'case-preview:recover-abandoned' as const;
const DEFAULT_BATCH_LIMIT = 50;
const DAY_1_AGE_HOURS = 24;
const DAY_7_AGE_HOURS = 24 * 7;
...
async function executeCasePreviewRecovery(request: NextRequest) {
  ...
  try {
    ...
    if (casesError) {
      throw new Error(`Failed to fetch preview-abandoned cases: ${casesError.message}`);
    }
    ...
    if (!caseIds.length) {
      await completeCronRun(run.id, 'success', 'No preview-abandoned case recovery candidates.', {
        sourcesChecked: 0,
        eventsCreated: 0,
        eventsUpdated: 0,
      });
    ...
    for (const caseRow of cases) {
      ...
      const finalEvent = await supabase.from('email_events').insert({
        ...
        event_type: emailResult.success
          ...
          : CASE_PREVIEW_RECOVERY_FAILED_EVENT_TYPES[dueStage],
        event_data: {
          case_id: caseRow.id,
          product_type: productType,
          resume_url: recovery.resumeUrl,
    ...
    const summary = dryRun
      ? `Dry run found ${checked} preview-abandoned case recovery candidates.`
      : `Sent ${emailsSent} preview-abandoned case recovery emails; skipped ${skipped}; failed ${failed}.`;
    ...
    const message = error instanceof Error ? error.message : 'Unknown case preview recovery error';
    await completeCronRun(run.id, 'failed', message, {
      sourcesChecked: 0,
      eventsUpdated: 0,
    });
    console.error('[CasePreviewRecoveryCron] Failed:', error);
...
export async function GET(request: NextRequest) {
  ...
  if (!authHeader && !vercelCronHeader) {
    return NextResponse.json({
      ...
      day_7_age_hours: DAY_7_AGE_HOURS,
      batch_limit: DEFAULT_BATCH_LIMIT,
      description:
        'Sends day-1 and day-7 restart emails for preview-abandoned cases with a known customer email.',
    });
  }

  return executeCasePreviewRecovery(request);
}
