...
const canonical = "https://landlordheaven.co.uk/how-to-evict-a-tenant-using-ground-12";
const groundCode = "12";
...
export const metadata: Metadata = {
  ...
  description: "Evict using Ground 12 for tenancy breaches. Check the 2-week notice, clause evidence, common mistakes, and landlord next steps before serving.",
...
const faqs: FAQItem[] = [
    { question: "What breaches can Ground 12 cover?", answer: "Ground 12 can cover non-rent breaches of tenancy terms, such as unauthorised occupiers, prohibited use, or refusing access where the agreement allows it." },
    { question: "Is Ground 12 discretionary?", answer: "Yes. The court considers whether possession is reasonable even where a breach is shown." },
    { question: "Is this a court approved Ground 12 notice?", answer: "Courts do not pre-approve notices. A validated Form 3A helps connect the clause, facts, and evidence, but the court decides the outcome." },
    { question: "Do I need the tenancy agreement?", answer: "Yes. Ground 12 usually depends on proving the term that was breached, so the agreement is central evidence." },
  ];
const relatedGrounds = [
    { code: "13", label: "Property deterioration", href: "/how-to-evict-a-tenant-using-ground-13" },
    { code: "14", label: "Antisocial behaviour", href: "/how-to-evict-a-tenant-using-ground-14" },
  ];
...
export default function Page() {
  return (
    <div className="min-h-screen bg-[#fcfaff]">
      ...
      <StructuredData data={articleSchema({ headline: "How to Evict a Tenant Using Ground 12 - Breach of Tenancy", description: metadata.description as string, url: canonical, datePublished: '2026-05-13', dateModified: '2026-05-13' })} />
      ...
      <main>
        <section className="border-b border-[#E6DBFF] bg-white py-6"><Container><div className="grid gap-4 md:grid-cols-3"><div className="rounded-xl border border-[#E6DBFF] bg-[#F8F4FF] p-4"><p className="text-sm font-semibold text-primary">Ground meaning</p><p className="mt-2 text-sm text-gray-700">Ground 12 is for breach of a tenancy term other than rent payment. It depends on the tenancy wording and the evidence of breach.</p></div><div className="rounded-xl border border-[#E6DBFF] bg-[#F8F4FF] p-4"><p className="text-sm font-semibold text-primary">Mandatory or discretionary status</p><p className="mt-2 text-sm text-gray-700">{groundLabel} is <strong>discretionary</strong>.</p></div><div className="rounded-xl border border-[#E6DBFF] bg-[#F8F4FF] p-4"><p className="text-sm font-semibold text-primary">Current notice period</p><p className="mt-2 text-sm text-gray-700">The current post-May 2026 notice period is <strong>2 weeks</strong>.</p></div></div></Container></section>
        <section className="py-12"><Container size="medium"><div className="space-y-8">
          <SectionCard title={"How landlords use Ground 12"}><p>Landlords use Ground 12 for breaches such as unauthorised subletting, access refusal, prohibited use, pets, smoking, or other express terms that have been broken.</p><p>Use our validated, solicitor-approved Notice Only route when you need a Form 3A notice and service pack. Use Complete Pack when court progression is likely and you want official court forms, evidence prompts, and a court-ready file prepared together.</p><p><Link href="/samples/notice-only" className="font-semibold text-primary hover:underline">See a real Form 3A notice with sample Ground {groundCode} evidence</Link>.</p></SectionCard>
          <SectionCard title="Post-May 2026 compliance note"><p>For post-May 2026 England cases, use Form 3A and give 2 weeks notice. The particulars should identify the tenancy clause, the facts of breach, and the evidence showing why possession is reasonable.</p><p>The notice must set out the substance of the ground and the reasons relied on. If the tenant does not leave, the landlord must prove the ground at court with a consistent notice, service record, and evidence bundle.</p></SectionCard>
          <SectionCard title={"Common mistakes with Ground 12"}><ul className="list-disc space-y-2 pl-5">{mistakes.map((mistake) => <li key={mistake}>{mistake}</li>)}</ul></SectionCard>
          <SectionCard title={"Ground 12 evidence checklist"}><p>Ground 12 evidence should connect the conduct to a specific tenancy term.</p><ul className="list-disc space-y-2 pl-5">{evidenceItems.map((item) => <li key={item}>{item}</li>)}</ul><p><Link href={"/checklists/ground-12.pdf"} className="font-semibold text-primary hover:underline">Download the ungated Ground {groundCode} PDF checklist</Link>.</p></SectionCard>
