...
const canonical = "https://landlordheaven.co.uk/how-to-evict-a-tenant-using-ground-17";
const groundCode = "17";
...
export const metadata: Metadata = {
  ...
  description: "Evict using Ground 17 for false statements. Check the 2-week notice, reliance evidence, mistakes, and landlord next steps before serving now.",
...
const faqs: FAQItem[] = [
    { question: "What kinds of false statement can Ground 17 cover?", answer: "It can cover material false information used to obtain the tenancy, such as false references, income, employment, identity, or household details." },
    { question: "Is Ground 17 discretionary?", answer: "Yes. The court considers the false statement evidence and whether possession is reasonable." },
    { question: "Is this a court approved Ground 17 notice?", answer: "Courts do not pre-approve notices. A validated Form 3A can help present the false statement evidence, but the court decides whether possession is reasonable." },
    { question: "Does every wrong application detail justify Ground 17?", answer: "No. The false statement needs to be material and linked to the grant of the tenancy. Small mistakes may not be enough." },
  ];
...
export default function Page() {
  return (
    <div className="min-h-screen bg-[#fcfaff]">
      ...
      <StructuredData data={articleSchema({ headline: "How to Evict a Tenant Using Ground 17 - False Statement by Tenant", description: metadata.description as string, url: canonical, datePublished: '2026-05-13', dateModified: '2026-05-13' })} />
      ...
      <main>
        ...
        <section className="py-12"><Container size="medium"><div className="space-y-8">
          <SectionCard title={"How landlords use Ground 17"}><p>Landlords use Ground 17 where dishonesty in the application process mattered to the decision to grant the tenancy, such as false income, identity, employment, references, or household information.</p><p>Use our validated, solicitor-approved Notice Only route when you need a Form 3A notice and service pack. Use Complete Pack when court progression is likely and you want official court forms, evidence prompts, and a court-ready file prepared together.</p><p><Link href="/samples/notice-only" className="font-semibold text-primary hover:underline">See a real Form 3A notice with sample Ground {groundCode} evidence</Link>.</p></SectionCard>
          <SectionCard title="Post-May 2026 compliance note"><p>For post-May 2026 England cases, use Form 3A and give 2 weeks notice. The evidence should prove the false statement, show it was knowing or reckless, and explain how it induced the tenancy grant.</p><p>The notice must set out the substance of the ground and the reasons relied on. If the tenant does not leave, the landlord must prove the ground at court with a consistent notice, service record, and evidence bundle.</p></SectionCard>
          <SectionCard title={"Common mistakes with Ground 17"}><ul className="list-disc space-y-2 pl-5">{mistakes.map((mistake) => <li key={mistake}>{mistake}</li>)}</ul></SectionCard>
          <SectionCard title={"Ground 17 evidence checklist"}><p>Ground 17 evidence should prove both the false statement and why it mattered.</p><ul className="list-disc space-y-2 pl-5">{evidenceItems.map((item) => <li key={item}>{item}</li>)}</ul><p><Link href={"/checklists/ground-17.pdf"} className="font-semibold text-primary hover:underline">Download the ungated Ground {groundCode} PDF checklist</Link>.</p></SectionCard>
