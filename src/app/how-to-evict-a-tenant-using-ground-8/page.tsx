...
const canonical = "https://landlordheaven.co.uk/how-to-evict-a-tenant-using-ground-8";
const groundCode = "8";
...
const faqs: FAQItem[] = [
    { question: "What is the current Ground 8 threshold?", answer: "Post-May 2026 guidance refers to at least 3 months rent arrears, or 13 weeks where rent is weekly or fortnightly, at both service and hearing." },
    { question: "Is Ground 8 mandatory?", answer: "Yes. Ground 8 is mandatory if the arrears threshold and legal requirements are proved, but the court still checks the evidence." },
    { question: "Is this a court approved Ground 8 notice?", answer: "Courts do not pre-approve notices. A validated, solicitor-approved Form 3A and arrears schedule help reduce mistakes, but the court decides whether Ground 8 is proved." },
    { question: "Why add Grounds 10 and 11?", answer: "Grounds 10 and 11 are discretionary arrears grounds. They can support the case if Ground 8 becomes vulnerable because the arrears reduce before hearing." },
  ];
...
export default function Page() {
  return (
    <div className="min-h-screen bg-[#fcfaff]">
      ...
      <StructuredData data={articleSchema({ headline: "How to Evict a Tenant Using Ground 8 - Serious Rent Arrears", description: metadata.description as string, url: canonical, datePublished: '2026-05-13', dateModified: '2026-05-13' })} />
      ...
      <main>
        <section className="border-b border-[#E6DBFF] bg-white py-6"><Container><div className="grid gap-4 md:grid-cols-3"><div className="rounded-xl border border-[#E6DBFF] bg-[#F8F4FF] p-4"><p className="text-sm font-semibold text-primary">Ground meaning</p><p className="mt-2 text-sm text-gray-700">Ground 8 is the mandatory rent arrears ground. It applies where arrears meet the required threshold at service and again at the possession hearing.</p></div><div className="rounded-xl border border-[#E6DBFF] bg-[#F8F4FF] p-4"><p className="text-sm font-semibold text-primary">Mandatory or discretionary status</p><p className="mt-2 text-sm text-gray-700">{groundLabel} is <strong>mandatory</strong>.</p></div><div className="rounded-xl border border-[#E6DBFF] bg-[#F8F4FF] p-4"><p className="text-sm font-semibold text-primary">Current notice period</p><p className="mt-2 text-sm text-gray-700">The current post-May 2026 notice period is <strong>4 weeks</strong>.</p></div></div></Container></section>
        <section className="py-12"><Container size="medium"><div className="space-y-8">
          <SectionCard title={"How landlords use Ground 8"}><p>Landlords use Ground 8 when arrears are serious enough to justify the mandatory route. It is often combined with Grounds 10 and 11 so the case does not collapse if payments reduce the arrears before hearing.</p><p>Use our validated, solicitor-approved Notice Only route when you need a Form 3A notice and service pack. Use Complete Pack when court progression is likely and you want official court forms, evidence prompts, and a court-ready file prepared together.</p><p><Link href="/samples/notice-only" className="font-semibold text-primary hover:underline">See a real Form 3A notice with sample Ground {groundCode} evidence</Link>.</p></SectionCard>
          <SectionCard title="Post-May 2026 compliance note"><p>For post-May 2026 England cases, use Form 3A, give 4 weeks notice, and check the higher arrears threshold. Keep a period-by-period rent schedule because the court will look at arrears at service and hearing.</p><p>The notice must set out the substance of the ground and the reasons relied on. If the tenant does not leave, the landlord must prove the ground at court with a consistent notice, service record, and evidence bundle.</p></SectionCard>
          <SectionCard title={"Common mistakes with Ground 8"}><ul className="list-disc space-y-2 pl-5">{mistakes.map((mistake) => <li key={mistake}>{mistake}</li>)}</ul></SectionCard>
          <SectionCard title={"Ground 8 evidence checklist"}><p>Ground 8 evidence should make the arrears threshold obvious and updateable.</p><ul className="list-disc space-y-2 pl-5">{evidenceItems.map((item) => <li key={item}>{item}</li>)}</ul><p><Link href={"/checklists/ground-8.pdf"} className="font-semibold text-primary hover:underline">Download the ungated Ground {groundCode} PDF checklist</Link>.</p></SectionCard>
