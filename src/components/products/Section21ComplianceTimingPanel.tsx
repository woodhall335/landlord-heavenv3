...
function Segmented({ value, onChange }: { value: YesNoUnsure; onChange: (v: YesNoUnsure) => void }) {
  return <div className="inline-flex rounded-full border border-[#7c3aed]/30 bg-white p-1 text-sm">{triState.map((option) => <button key={option} type="button" onClick={() => onChange(option)} className={`rounded-full px-3 py-1.5 font-medium capitalize ${value === option ? 'bg-[#7c3aed] text-white' : 'text-[#7c3aed]'}`}>{option}</button>)}</div>;
}
...
export default function Section21ComplianceTimingPanel() {
  ...
  const [gateOpen, setGateOpen] = useState(
    typeof window !== 'undefined' && (isLeadCaptured() || localStorage.getItem('lh_s21_precheck_gate') === '1')
  );
  ...
  return (
    <div className="mx-auto mb-8 w-full max-w-[800px] rounded-2xl border-l-4 border-[#7c3aed] bg-white p-5 shadow-sm md:p-6">
      <h3 className="text-2xl font-bold text-gray-900">⚠️ Quick compliance & timing check</h3>
      <p className="mt-2 text-gray-600">One mistake can invalidate a Section 21 notice. Check in 60 seconds.</p>
      <div className="mt-5 space-y-5">
        <div><p className="mb-2 font-semibold">A) Compliance / prerequisites</p><div className="grid gap-3 md:grid-cols-2">{[['deposit_taken', 'Deposit taken?'], ['epc_required', 'EPC required?'], ['gas_installed', 'Gas installed?'], ['property_requires_hmo_licence', 'HMO licence required?'], ['property_requires_selective_licence', 'Selective licence required?'], ['improvement_notice_served', 'Improvement notice served?'], ['emergency_remedial_action_served', 'Emergency remedial action served?'], ['prohibited_payment_outstanding', 'Prohibited payment outstanding?'], ['has_proof_of_service_plan', 'Proof-of-service plan in place?'], ['is_replacement_tenancy', 'Replacement tenancy?']].map(([key, label]) => <div key={key} className="flex items-center justify-between gap-3 rounded-lg border border-gray-100 p-2"><span className="text-sm">{label}</span><Segmented value={input[key as keyof Section21PrecheckInput] as YesNoUnsure} onChange={(v) => update(key as keyof Section21PrecheckInput, v as never)} /></div>)}</div></div>
        ...
        <div className="rounded-xl border border-gray-200 bg-[#faf7ff] p-4"><p className="text-sm font-semibold">Status preview</p><div className="mt-2 flex items-center gap-3"><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-[#7c3aed] border border-[#7c3aed]/30">{statusBadge}</span><span className="text-sm text-gray-700">{result?.display.gatedSummary ?? 'Loading...'}</span></div>
          {result && result.status !== 'incomplete' && !gateOpen && <button type="button" onClick={() => setOpenModal(true)} className="mt-3 rounded-lg bg-[#7c3aed] px-4 py-2 text-sm font-semibold text-white">Reveal full results</button>}
          ...
          <Link href={result?.display.ctaHref ?? '/wizard/flow?type=eviction&jurisdiction=england&product=notice_only&src=product_page&topic=eviction'} className="mt-4 block w-full rounded-lg bg-gradient-to-r from-[#692ed4] to-[#7c3aed] px-4 py-3 text-center font-semibold text-white">{result?.display.ctaLabel ?? 'Section 21 is Valid – Continue'}</Link>
        </div>
      </div>

      <EmailCaptureModal open={openModal} onClose={() => setOpenModal(false)} source="s21_precheck_results_gate" tags={['s21_precheck', 'product_notice_only', 'england']} includeEmailReport={false} title="Reveal your full Section 21 pre-check results" description="Enter your email to unlock detailed reasons, date calculations, and next-step guidance." primaryLabel="Reveal results" onSuccess={() => { setGateOpen(true); if (typeof window !== 'undefined') localStorage.setItem('lh_s21_precheck_gate', '1'); }} />
    </div>
  );
}
