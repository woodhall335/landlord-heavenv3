...
export interface ValidatorPageProps {
  ...
  /**
   * Restrict jurisdiction selector to specific options.
   * If provided, only these jurisdictions will be available in the dropdown.
   * For England-only validators (Section 21, Section 8), pass ['england'].
   */
...
export function ValidatorPage({
  ...
  // Filter jurisdiction options if allowedJurisdictions is specified
  const availableJurisdictions = allowedJurisdictions
    ? JURISDICTION_OPTIONS.filter((opt) => allowedJurisdictions.includes(opt.value))
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section - hidden when SSR content provides the H1/intro above */}
      {!hideHeroSection && (
        ...
        <Container>
          <div className="max-w-4xl mx-auto">
            <div id="validator" className="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
              ...
              {/* Jurisdiction Selector (when needed and multiple options available) */}
              {needsJurisdictionSelector && !selectedJurisdiction && availableJurisdictions.length > 1 && (
                <div className="mb-6">
              ...
              {caseId && !isLoading && effectiveJurisdiction && (
                <>
                  {needsJurisdictionSelector && availableJurisdictions.length > 1 && (
                    <div className="mb-4 flex items-center justify-between rounded-lg bg-gray-50 px-4 py-2">
                      <span className="text-sm text-gray-600">
                        Validating for: <strong className="text-gray-900">{availableJurisdictions.find(j => j.value === effectiveJurisdiction)?.label}</strong>
                      </span>
                      <button
                        type="button"
                        onClick={() => {
