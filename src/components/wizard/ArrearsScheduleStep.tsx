...
export const ArrearsScheduleStep: React.FC<ArrearsScheduleStepProps> = ({ facts, onUpdate, jurisdiction = 'england' }) => {
  ...
  // - facts.notice_service_date / facts.notice_served_date (eviction wizard - actual service date aliases)
  const noticeDate =
  ...
  // Helper to update facts with computed arrears (defined early for use in effects)
  const updateFacts = useCallback((items: ArrearsItem[]) => {
    ...
    onUpdate({
      ...
      rent_arrears_amount: computed.total_arrears,
  ...
  }, [rentFrequency, rentAmount, issues, rentArrears, onUpdate]);
  ...
  useEffect(() => {
    if (canGeneratePeriods && arrearsItems.length === 0 && !hasAutoGenerated.current) {
      ...
      if (newItems.length > 0) {
        ...
        const computed = computeArrears(newItems, rentFrequency, rentAmount);
    ...
    canGeneratePeriods,
    scheduleInput.tenancy_start_date,
    scheduleInput.rent_amount,
    scheduleInput.rent_frequency,
    scheduleInput.notice_date,
    arrearsItems.length,
    rentFrequency,
    rentAmount,
    issues,
  ...
  // Track previous notice date for comparison
  const prevNoticeDateRef = useRef<string | undefined>(noticeDate || undefined);

  // Update final period's rent_due when notice_date changes (pro-rata recalculation)
  // This ensures the final period is correctly pro-rated without losing payment status data
  useEffect(() => {
    const currentNoticeDate = noticeDate || undefined;
    const prevNoticeDate = prevNoticeDateRef.current;

    // Only run if notice_date has actually changed and we have existing items
    if (currentNoticeDate === prevNoticeDate || arrearsItems.length === 0) {
      prevNoticeDateRef.current = currentNoticeDate;
      return;
    }

    prevNoticeDateRef.current = currentNoticeDate;

    // Generate fresh periods with the new notice_date to get correct pro-rata calculations
    const freshPeriods = createEmptyArrearsSchedule(scheduleInput);
    if (freshPeriods.length === 0) return;

    // Get the last period from fresh generation (correctly pro-rated)
    const freshFinalPeriod = freshPeriods[freshPeriods.length - 1];

    // Find the matching period in existing items
    const existingFinalIndex = arrearsItems.findIndex(
      (item) => item.period_start === freshFinalPeriod.period_start
    );

    if (existingFinalIndex === -1) {
      // If no matching period found, we may need to add/remove periods - use full regeneration
      // but preserve payment data for matching periods
      const updatedItems = freshPeriods.map((freshItem) => {
        const existingItem = arrearsItems.find((e) => e.period_start === freshItem.period_start);
        if (existingItem) {
          // Preserve payment data but update rent_due and period_end
          const rentPaid = Math.min(existingItem.rent_paid, freshItem.rent_due);
          return {
            ...freshItem,
            rent_paid: rentPaid,
            amount_owed: freshItem.rent_due - rentPaid,
          };
        }
        return freshItem;
      });
      ...
      arrearsItems[existingFinalIndex].rent_due !== freshFinalPeriod.rent_due ||
      arrearsItems[existingFinalIndex].period_end !== freshFinalPeriod.period_end
    ) {
      // Update only the final period to preserve all other payment data
      const updatedItems = [...arrearsItems];
      const existingItem = updatedItems[existingFinalIndex];

      // Preserve rent_paid but cap it at the new (potentially lower) rent_due
      const newRentDue = freshFinalPeriod.rent_due;
      const adjustedRentPaid = Math.min(existingItem.rent_paid, newRentDue);

      updatedItems[existingFinalIndex] = {
        ...existingItem,
        period_end: freshFinalPeriod.period_end,
        rent_due: newRentDue,
        rent_paid: adjustedRentPaid,
        amount_owed: newRentDue - adjustedRentPaid,
        is_pro_rated: freshFinalPeriod.is_pro_rated,
        days_in_period: freshFinalPeriod.days_in_period,
        notes: freshFinalPeriod.notes,
      };
  ...
  }, [noticeDate, scheduleInput.tenancy_start_date, scheduleInput.rent_amount, scheduleInput.rent_frequency, updateFacts]);
  ...
  }, [canGeneratePeriods, scheduleInput, rentFrequency, rentAmount, issues, rentArrears, onUpdate]);
