...
function extractCasePosition(text: string): string {
  const match = text.match(/Case position:\s*(.*?)\s*Use this document:/);
...
describe('Section 13 document generation hardening', () => {
  ...
  it('uses tribunal-ready empty-evidence wording and Section 13 proof-of-service branding', async () => {
    ...
    expect(proofText).toContain('Generated by Landlord Heaven Section 13 Wizard');
  ...
  it('elevates the Standard cover letter with clearer next steps', async () => {
    ...
    expect(coverText).toContain('Purpose: This cover letter explains what is in the service pack and how to use it before serving Form 4A.');
    expect(coverText).toContain('This pack sets out a complete and evidence-backed proposal for the revised rent.');
    expect(coverText).toContain('Case position: This proposal is grounded in');
  ...
  it('adds the Tribunal Argument Summary and places it directly after the justification report in the bundle', async () => {
    ...
    expect(argumentText).toContain("Purpose: This is the landlord's concise argument brief for the hearing file.");
    expect(argumentText).toContain('Case position: The proposed rent of £1285.00 per month is supported by');
  ...
  it('adds premium key summary blocks and distinct case-position phrasing across narrative documents', async () => {
    ...
    const casePositions = [
      extractCasePosition(coverText),
      extractCasePosition(justificationText),
      extractCasePosition(argumentText),
      extractCasePosition(defenceText),
    ];

    expect(new Set(casePositions).size).toBe(casePositions.length);
  }, 120000);
});
