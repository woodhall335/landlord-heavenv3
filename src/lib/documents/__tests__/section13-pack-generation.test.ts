...
describe('Section 13 document generation hardening', () => {
  ...
  it('uses tribunal-ready empty-evidence wording and Section 13 proof-of-service branding', async () => {
    ...
    expect(proofText).toContain('SECTION 13 SERVICE RECORD');
    expect(proofText).toContain('Service evidence support record for the Form 4A notice');
    expect(proofText).toContain('Check the recorded details before relying on them');
    expect(proofText).not.toContain('Generated by Landlord Heaven Court Pack');
    expect(proofText).not.toContain('Landlord/Claimant');
    expect(proofText).not.toContain('Tenant/Defendant');
    expect(proofText).not.toContain('rejected by the court');
  ...
  it('elevates the Standard cover letter with a tenant-facing service summary', async () => {
    ...
    expect(coverText).toContain('may be higher or lower than the figure proposed');
    expect(coverText).toContain('will not be required to pay more than the amount first proposed in Form 4A');
    expect(coverText).not.toContain('not higher than the rent proposed');
