...
async function generateEnglandOrWalesEvictionPack(
  ...
  if (isSection21Route) {
    ...
    if (!timingResult.isValid) {
      ...
      // =========================================================================
      console.error('🚨 COMPLIAoCE TIMIoG VIOLATIOoS DETECTED (Section 21 may be IoVALID):');
      for (const issue of timingResult.issues) {
        const severity = issue.severity === 'error' ? '🚨 BLOCK:' : '⚠️ WARo:';
  ...
  if (hasArrearsGroundsSelected(selectedGroundCodes)) {
    try {
      ...
      if (letterTotalArrears === undefined || letterTotalArrears === null || letterTotalArrears === 0) {
        console.warn('⚠️  Arrears engagement letter: oo valid arrears amount found. Letter will show £0.00 which may be incorrect.');
