...
export function calculateSection8ExpiryDate(params: Section8DateParams): DateCalculationResult {
  ...
  // Add notice period to service date
  const expiryDateObj = new Date(serviceDateObj);
  expiryDateObj.setDate(expiryDateObj.getDate() + notice_period_days);

  // If fixed term, expiry date cannot be before fixed term ends
  const warnings: string[] = [];
  if (fixed_term && fixed_term_end_date) {
    const fixedTermEndObj = new Date(fixed_term_end_date);
    if (expiryDateObj < fixedTermEndObj) {
      expiryDateObj.setTime(fixedTermEndObj.getTime());
      warnings.push(
  ...
  const earliest_valid_date = expiryDateObj.toISOString().split('T')[0];
...
export function validateSection8ExpiryDate(
  ...
  const proposedDateObj = new Date(proposed_date);
  const earliestDateObj = new Date(calculatedResult.earliest_valid_date);
 ...
 *
 * Rules:
 * 1. Minimum 2 months notice (60 days)
...
export function calculateSection21ExpiryDate(params: Section21DateParams): DateCalculationResult {
  ...
  if (expiryDateObj < fourMonthsAfterStart) {
    expiryDateObj = fourMonthsAfterStart;
  ...
  if (!fixed_term || periodic_tenancy_start) {
    const periodStart = periodic_tenancy_start ? new Date(periodic_tenancy_start) : tenancyStartObj;
  ...
  return {
    ...
    explanation,
    legal_basis: 'Housing Act 1988, Section 21(4) - Minimum 2 months notice',
    warnings,
  };
}
...
export function validateSection21ExpiryDate(
  ...
  const tenancyStartObj = new Date(params.tenancy_start_date);
  const fourMonthsAfterStart = new Date(tenancyStartObj);
  fourMonthsAfterStart.setMonth(fourMonthsAfterStart.getMonth() + 4);
...
export function calculateNoticeToLeaveDate(params: NoticeToLeaveDateParams): DateCalculationResult {
  ...
  // Add notice period to service date
  const leavingDateObj = new Date(serviceDateObj);
  leavingDateObj.setDate(leavingDateObj.getDate() + notice_period_days);

  const earliest_valid_date = leavingDateObj.toISOString().split('T')[0];
...
/**
 * Align a date with the end of a rent period
 */
function alignWithRentPeriod(
  ...
  switch (rentPeriod) {
    case 'weekly': {
      // Align to same day of week as period start
      const dayOfWeek = periodStart.getDay();
      const targetDayOfWeek = aligned.getDay();
      const daysToAdd = (dayOfWeek - targetDayOfWeek + 7) % 7;
      if (daysToAdd > 0) aligned.setDate(aligned.getDate() + daysToAdd);
      break;
    }
    case 'fortnightly': {
    ...
    case 'monthly': {
      // Align to same day of month as period start
      const dayOfMonth = periodStart.getDate();
      aligned.setDate(dayOfMonth);
    ...
    case 'quarterly': {
      // Align to same day, 3 months ahead
      const dayOfMonth = periodStart.getDate();
      aligned.setDate(dayOfMonth);
      while (aligned < targetDate) {
        aligned.setMonth(aligned.getMonth() + 3);
      }
      break;
    }
    case 'yearly': {
      // Align to same day/month, next year
      aligned.setMonth(periodStart.getMonth());
      aligned.setDate(periodStart.getDate());
      while (aligned < targetDate) {
