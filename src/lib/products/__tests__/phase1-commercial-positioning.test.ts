...
describe('Phase 1 commercial positioning', () => {
  ...
  it('updates Notice Only and Complete Pack value framing for the new pricing and service position', () => {
    const noticeSource = readSource('src/app/products/notice-only/page.tsx');
    const completePackSource = readSource('src/app/products/complete-pack/page.tsx');
    const rentIncreaseHubSource = readSource('src/app/rent-increase/page.tsx');

    expect(noticeSource).toContain('Review-ready Section 8 notice and service file');
    ...
    expect(completePackSource).toContain('Review-ready Section 8 court and possession file');
    expect(completePackSource).toContain('Prepare the full possession file, not just the court forms');
    expect(completePackSource).toContain('/images/illustrations/products/possession-court-pack-waterbrush-v1.webp');
    expect(completePackSource).toContain('N5, N119, witness statement, and evidence collection checklist');
  ...
  it('keeps money-claim product copy transactional rather than broad-owner led', () => {
    const source = readSource('src/app/products/money-claim/page.tsx');
