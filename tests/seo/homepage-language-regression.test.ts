...
describe('Homepage language regression', () => {
  it('removes internal-sounding homepage copy and keeps review proof aligned', () => {
    ...
    expect(homepage).toContain('const reviewSummary = `${REVIEW_RATING}/5 | ${reviewCount} reviews`;');
    expect(homepage).toContain('The same rating and review count shown in the hero trust pill.');
